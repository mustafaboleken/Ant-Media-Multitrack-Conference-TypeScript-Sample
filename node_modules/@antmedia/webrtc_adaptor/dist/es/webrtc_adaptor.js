import { _ as _defineProperty } from './_rollupPluginBabelHelpers-dd316e1a.js';
import './loglevel.min-bf7f00e7.js';
...
class WebSocketAdaptor {
    ...
    */
    ...
    this.wsConn.onmessage = event => {
      ...
      if (obj.command == "start") {
        ...
        if (this.debug) {
          Logger$3.debug("received start command");
        }
        this.webrtcadaptor.startPublishing(obj.streamId);
      } else if (obj.command == "takeCandidate") {
        if (this.debug) {
          Logger$3.debug("received ice candidate for stream id " + obj.streamId);
          Logger$3.debug(obj.candidate);
        }
        this.webrtcadaptor.takeCandidate(obj.streamId, obj.label, obj.candidate);
      } else if (obj.command == "takeConfiguration") {
        if (this.debug) {
          Logger$3.debug("received remote description type for stream id: " + obj.streamId + " type: " + obj.type);
        }
        this.webrtcadaptor.takeConfiguration(obj.streamId, obj.sdp, obj.type, obj.idMapping);
      } else if (obj.command == "stop") {
        if (this.debug) {
          Logger$3.debug("Stop command received");
        }
    ...
    this.wsConn.onerror = error => {
      this.connecting = false;
      this.connected = false;
      Logger$3.info(" error occured: " + JSON.stringify(error));
      this.clearPingTimer();
    ...
    this.wsConn.onclose = event => {
      ...
      this.connected = false;
      if (this.debug) {
        Logger$3.debug("connection closed.");
      }
      this.clearPingTimer();
  ...
  clearPingTimer() {
    if (this.pingTimerId != -1) {
      if (this.debug) {
        Logger$3.debug("Clearing ping message timer");
      }
  ...
  send(text) {
    ...
    try {
      ...
      if (this.debug) {
        Logger$3.debug("sent message:" + text);
      }
    } catch (error) {
      Logger$3.warn("Cannot send message:" + text);
    }
  }
  isConnected() {
    ...
    return this.context.audioWorklet.addModule(new URL('./volume-meter-processor.js', import.meta.url)).then(() => {
      ...
      this.volumeMeterNode.port.onmessage = event => {
        if (event.data.type == 'debug') {
          Logger$2.debug(event.data.message);
        } else {
          this.instant = event.data;
          levelCallback(this.instant.toFixed(2));
          Logger$2.debug("Audio level: " + this.instant.toFixed(2));
        }
      };
      ...
      Logger$2.error("Error in soundmeter: " + err);
      throw err;
    });
  }
  stop() {
...
class MediaManager {
  ...
  constructor(initialValues) {
    ...
    this.callback = (info, obj) => {
      Logger$1.debug("Callback info: " + info + " object: " + typeof obj !== undefined ? JSON.stringify(obj) : "");
    };
  ...
  initLocalStream() {
    ...
    if (typeof this.mediaConstraints.video != "undefined" && this.mediaConstraints.video != false) {
      ...
      Logger$1.debug("no media requested, just return an empty stream");
  ...
  */
  checkWebRTCPermissions() {
    if (!("WebSocket" in window)) {
      Logger$1.debug("WebSocket not supported.");
      this.callbackError("WebSocketNotSupported");
      return;
    }
    if (typeof navigator.mediaDevices == "undefined") {
      Logger$1.debug("Cannot open camera and mic because of unsecure context. Please Install SSL(https)");
  ...
  getDevices() {
    return navigator.mediaDevices.enumerateDevices().then(devices => {
      ...
      if (checkAudio == false && this.localStream == null) {
        Logger$1.debug("Audio input not found");
        Logger$1.debug("Retrying to get user media without audio");
        if (this.inputDeviceNotFoundLimit < 2) {
          if (checkVideo != false) {
            ...
            Logger$1.debug("Video input not found");
      ...
      Logger$1.error("Cannot get devices -> error name: " + err.name + ": " + err.message);
      throw err;
    });
  }
  ...
  navigatorDisplayMedia(mediaConstraints, func) {
    return navigator.mediaDevices.getDisplayMedia(mediaConstraints).then(stream => {
      ...
      if (error.name === "NotAllowedError") {
        Logger$1.debug("Permission denied error");
  ...
  enableAudioLevelWhenMuted() {
    navigator.mediaDevices.getUserMedia({
      ...
      Logger$1.debug("Can't get the soundlevel on mute");
    });
  }
  ...
  * @Deprecated. It's not the job of SDK to make these things. It increases the complexity of the code. 
  ...
  mixAudioStreams(stream, secondStream) {
    ...
    if (stream.getAudioTracks().length > 0) {
      ...
      Logger$1.debug("Origin stream does not have audio track");
    }
    if (secondStream.getAudioTracks().length > 0) {
      ...
      Logger$1.debug("Second stream does not have audio track");
    }
    audioDestionation.stream.getAudioTracks().forEach(function (track) {
      composedStream.addTrack(track);
      Logger$1.debug("audio destination add track");
    });
  ...
  switchAudioInputSource(streamId, deviceId) {
    ...
    if (audioTrack) {
      ...
      Logger$1.warn("There is no audio track in local stream");
    }
  ...
  switchVideoCameraCapture(streamId, deviceId, onEndedCallback) {
    ...
    if (this.localStream && this.localStream.getVideoTracks().length > 0) {
      ...
      Logger$1.warn("There is no video track in local stream");
    }
    this.publishMode = "camera";
    return navigator.mediaDevices.enumerateDevices().then(devices => {
      ...
      Logger$1.debug("Given deviceId = " + deviceId + " - Media constraints video property = " + this.mediaConstraints.video);
  ...
  switchVideoCameraFacingMode(streamId, facingMode) {
    ...
    this.publishMode = "camera";
    Logger$1.debug("Media constraints video property = " + this.mediaConstraints.video);
